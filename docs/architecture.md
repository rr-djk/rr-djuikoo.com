...
- **Outils d'exploration** (`makeExplorerTools`) : `list_files`, `read_file` (tronqué à 30 Ko), `search_code` (recherche de texte littéral insensible à la casse, alternatives séparées par `|`, limitée à 40 correspondances ; jamais d'expression régulière, pour qu'un motif fourni par le modèle ne puisse pas bloquer la Lambda).
